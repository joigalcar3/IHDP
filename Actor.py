...
class Actor:
    def __init__(self, selected_inputs, selected_states, tracking_states, number_time_steps, layers=(6, 1),
                 activations=('relu', 'linear'), batch_size=1, epochs=1, learning_rate=10,
                 learning_rate_exponent_limit=10, WB_limits=30):
    ...
    def train_actor_online(self, Jt1, dJt1_dxt1, G):
        ...
        """
        Jt1 = Jt1.flatten()[0]
        # dJt1_dxt1 = np.reshape(dJt1_dxt1, [-1, 1])
        # dJt1_dxt1 = np.reshape(dJt1_dxt1[:self.number_states, :], [-1, 1])

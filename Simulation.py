...
class Simulation:
    ...
    def plot_q_difference(self, max_x_frame):
        """
        Plots the vehicle pitch rate and the reference pitch rate generated by the cascaded actor network
        :param max_x_frame: the number of time steps ahead of the current time-step to be plotted.
        :return:
        """
        time = np.reshape(self.time[:self.time_step + max_x_frame], [1, -1]).T
        ...
        plt.clf()
        q_index = self.selected_states.index('q')
        q = np.reshape(self.system.store_states[q_index, :min(len(self.time), self.time_step + max_x_frame)], [-1, 1])
